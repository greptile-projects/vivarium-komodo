...
func main() {
	...
	agentProjectRoot := os.Getenv("AGENT_PROJECT_ROOT")
	if agentProjectRoot == "" {
		agentProjectRoot = "data/agent-projects"
	}
	agentProjectStore, err := agentprojects.New(agentProjectRoot)
	if err != nil {
		log.Fatal(err)
	}
	workflowDefinitionRoot := os.Getenv("WORKFLOW_DEFINITION_ROOT")
	if workflowDefinitionRoot == "" {
		workflowDefinitionRoot = "data/workflow-definitions"
	}
	workflowDefinitionStore, err := workflowdefinitions.New(workflowDefinitionRoot)
	if err != nil {
		log.Fatal(err)
	}
	workflowComponentRoot := os.Getenv("WORKFLOW_COMPONENT_ROOT")
	if workflowComponentRoot == "" {
		workflowComponentRoot = "data/workflow-components"
	}
	workflowComponentStore, err := workflowcomponents.New(workflowComponentRoot)
	if err != nil {
		log.Fatal(err)
	}
	agentScenarioRoot := os.Getenv("AGENT_SCENARIO_ROOT")
	if agentScenarioRoot == "" {
		agentScenarioRoot = "data/agent-scenarios"
	}
	agentScenarioStore, err := agentscenarios.New(agentScenarioRoot)
	if err != nil {
		log.Fatal(err)
	}
	...
	reviewPlanRoot := os.Getenv("REVIEW_PLAN_ROOT")
	if reviewPlanRoot == "" {
		reviewPlanRoot = "data/review-plans"
	}
	reviewPlanStore, err := reviewplans.New(reviewPlanRoot)
	if err != nil {
		log.Fatal(err)
	}
	reviewRoutingRoot := os.Getenv("REVIEW_ROUTING_ROOT")
	if reviewRoutingRoot == "" {
	...
	provenanceGraphRoot := os.Getenv("PROVENANCE_GRAPH_ROOT")
	if provenanceGraphRoot == "" {
	...
	provenanceAssessmentRoot := os.Getenv("PROVENANCE_ASSESSMENT_ROOT")
	if provenanceAssessmentRoot == "" {
		provenanceAssessmentRoot = "data/provenance-assessments"
	}
	provenanceAssessmentStore, err := provenanceassessments.New(provenanceAssessmentRoot)
	if err != nil {
		log.Fatal(err)
	}
	...
	propagationCampaignRoot := os.Getenv("PROPAGATION_CAMPAIGN_ROOT")
	if propagationCampaignRoot == "" {
	...
	observabilityGapRoot := os.Getenv("OBSERVABILITY_GAP_ROOT")
	if observabilityGapRoot == "" {
		observabilityGapRoot = "data/observability-gaps"
	}
	observabilityGapStore, err := observabilitygaps.New(observabilityGapRoot)
	if err != nil {
		log.Fatal(err)
	}
	signalContractRoot := os.Getenv("SIGNAL_CONTRACT_ROOT")
	if signalContractRoot == "" {
		signalContractRoot = "data/signal-contracts"
	}
	signalContractStore, err := signalcontracts.New(signalContractRoot)
	if err != nil {
		log.Fatal(err)
	}
	signalImplementationRoot := os.Getenv("SIGNAL_IMPLEMENTATION_ROOT")
	if signalImplementationRoot == "" {
	...
	signalEvaluationRoot := os.Getenv("SIGNAL_EVALUATION_ROOT")
	if signalEvaluationRoot == "" {
		signalEvaluationRoot = "data/signal-evaluations"
	}
	signalEvaluationStore, err := signalevaluations.New(signalEvaluationRoot)
	if err != nil {
		log.Fatal(err)
	}
	capacityModelRoot := os.Getenv("CAPACITY_MODEL_ROOT")
	if capacityModelRoot == "" {
		capacityModelRoot = "data/capacity-models"
	}
	capacityModelStore, err := capacitymodels.New(capacityModelRoot)
	if err != nil {
		log.Fatal(err)
	}
	capacityRehearsalRoot := os.Getenv("CAPACITY_REHEARSAL_ROOT")
	if capacityRehearsalRoot == "" {
	...
	capacityDeliveryRoot := os.Getenv("CAPACITY_DELIVERY_ROOT")
	if capacityDeliveryRoot == "" {
	...
	responseRotationRoot := os.Getenv("RESPONSE_ROTATION_ROOT")
	if responseRotationRoot == "" {
		responseRotationRoot = "data/response-rotations"
	}
	responseRotationStore, err := responserotations.New(responseRotationRoot)
	if err != nil {
		log.Fatal(err)
	}
	responseAlertRoot := os.Getenv("RESPONSE_ALERT_ROOT")
	if responseAlertRoot == "" {
		responseAlertRoot = "data/response-alerts"
	}
	responseAlertStore, err := responsealerts.New(responseAlertRoot)
	if err != nil {
		log.Fatal(err)
	}
	responseOutcomeRoot := os.Getenv("RESPONSE_OUTCOME_ROOT")
	if responseOutcomeRoot == "" {
		responseOutcomeRoot = "data/response-outcomes"
	}
	responseOutcomeStore, err := responseoutcomes.New(responseOutcomeRoot)
	if err != nil {
		log.Fatal(err)
	}
	runbookRoot := os.Getenv("RUNBOOK_ROOT")
	if runbookRoot == "" {
	...
	runbookRehearsalRoot := os.Getenv("RUNBOOK_REHEARSAL_ROOT")
	if runbookRehearsalRoot == "" {
		runbookRehearsalRoot = "data/runbook-rehearsals"
	}
	runbookRehearsalStore, err := runbookrehearsals.New(runbookRehearsalRoot)
	if err != nil {
		log.Fatal(err)
	}
	...
	historyRemediationRoot := os.Getenv("HISTORY_REMEDIATION_ROOT")
	if historyRemediationRoot == "" {
		historyRemediationRoot = "data/history-remediations"
	}
	...
	projectIncubatorRoot := os.Getenv("PROJECT_INCUBATOR_ROOT")
	if projectIncubatorRoot == "" {
		projectIncubatorRoot = "data/project-incubators"
	}
	projectIncubatorStore, err := projectincubators.New(projectIncubatorRoot)
	if err != nil {
		log.Fatal(err)
	}
	projectBoundaryRoot := os.Getenv("PROJECT_BOUNDARY_ROOT")
	if projectBoundaryRoot == "" {
		projectBoundaryRoot = "data/project-boundaries"
	}
	projectBoundaryStore, err := projectboundaries.New(projectBoundaryRoot)
	if err != nil {
		log.Fatal(err)
	}
	...
	adoptionWorkspaceRoot := os.Getenv("ADOPTION_WORKSPACE_ROOT")
	if adoptionWorkspaceRoot == "" {
		adoptionWorkspaceRoot = "data/adoption-workspaces"
	}
	...
	learningPathwayRoot := os.Getenv("LEARNING_PATHWAY_ROOT")
	if learningPathwayRoot == "" {
		learningPathwayRoot = "data/learning-pathways"
	}
	learningPathwayStore, err := learningpathways.New(learningPathwayRoot)
	if err != nil {
		log.Fatal(err)
	}
	learningOutcomeRoot := os.Getenv("LEARNING_OUTCOME_ROOT")
	if learningOutcomeRoot == "" {
		learningOutcomeRoot = "data/learning-outcomes"
	}
	learningOutcomeStore, err := learningoutcomes.New(learningOutcomeRoot)
	if err != nil {
		log.Fatal(err)
	}
	...
	registerGitHTTP(mux, repositoryCatalog, credentials, historyRemediationStore)
	...
	registerSignalEvaluationsHTTP(mux, signalEvaluationStore, observabilityGapStore, repositoryCatalog, credentials)
	...
	registerAdoptionWorkspacesHTTP(mux, adoptionWorkspaceStore, credentials)
	...
	registerLearningOutcomesHTTP(mux, learningOutcomeStore, repositoryCatalog, credentials)
	...
	registerAgentProjectsHTTP(mux, agentProjectStore, repositoryCatalog, credentials)
	registerWorkflowDefinitionsHTTP(mux, workflowDefinitionStore, repositoryCatalog, credentials)
	registerWorkflowComponentsHTTP(mux, workflowComponentStore, repositoryCatalog, credentials)
	registerAgentScenariosHTTP(mux, agentScenarioStore, repositoryCatalog, credentials)
	registerAgentDiscoveryHTTP(mux, agentDiscoveryStore, agentProfileStore, repositoryCatalog, credentials)
	registerAgentEvaluationsHTTP(mux, agentEvaluationStore, agentProfileStore, repositoryCatalog, organizationStore, credentials, agentEvaluationSources{projects: agentProjectStore, pulls: pullRequestStore})
