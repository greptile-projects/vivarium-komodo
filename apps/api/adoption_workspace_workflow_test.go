...
func TestAdoptionWorkspaceWorkflow(t *testing.T) {
	...
	workflowJSON(t, server.URL, http.MethodPost, observationPath, maintainer, `{"consumer_revision":"consumer-commit-81","kind":"access","status":"revoked","summary":"provider withdrew the release attestation","evidence_reference":"attestation:revocation-9"}`, http.StatusCreated, &workspace)
	if workspace.Candidates[0].Deliveries[0].Status != "paused" {
		t.Fatalf("revoked access did not pause adoption")
	}
	workflowJSON(t, server.URL, http.MethodPost, observationPath, agent, `{"consumer_revision":"consumer-commit-81","kind":"rollout","status":"restored","summary":"agent chose rollback","evidence_reference":"deployment:rollback-81"}`, http.StatusForbidden, nil)
	workflowJSON(t, server.URL, http.MethodPost, observationPath, owner, `{"consumer_revision":"consumer-commit-81","kind":"rollout","status":"restored","summary":"consumer owner restored the attested prior release","evidence_reference":"deployment:rollback-81"}`, http.StatusCreated, &workspace)
	...
	workflowJSON(t, server.URL, http.MethodPost, contributionPath, agent, `{"share_ids":["`+share+`"],"kind":"fork_pull_request","repository_id":"federated:provider/compiler","resource_id":"pull:provider/compiler#19","revision":"repair-commit-19","author_kind":"agent","author_id":"agent:fit-reader","contributor_guidance":"pathway:compiler-v4","review_reference":"review:19","checks_reference":"checks:19","security_reference":"security:19","local_resolution":"retain the consumer compatibility flag until an accepted release is verified"}`, http.StatusCreated, &workspace)
	...
	workflowJSON(t, server.URL, http.MethodPost, contributionPath+"/"+contribution+"/decision", maintainer, `{"decision":"accepted","reason":"ordinary review, checks, and security policy passed","release":"v2.1.1","revision":"repair-commit-19"}`, http.StatusCreated, &workspace)
	...
	workflowJSON(t, server.URL, http.MethodPost, updatePath, maintainer, `{"contribution_id":"`+contribution+`","delivery_id":"`+delivered.ID+`","from_consumer_revision":"consumer-commit-81","to_consumer_revision":"consumer-commit-92","accepted_release":"v2.1.1","provider_revision":"repair-commit-19","replaced_patches":["temporary module compatibility flag"],"evidence":`+updateEvidence+`}`, http.StatusForbidden, nil)
	workflowJSON(t, server.URL, http.MethodPost, updatePath, owner, `{"contribution_id":"`+contribution+`","delivery_id":"`+delivered.ID+`","from_consumer_revision":"consumer-commit-81","to_consumer_revision":"consumer-commit-92","accepted_release":"v2.1.1","provider_revision":"repair-commit-19","replaced_patches":["temporary module compatibility flag"],"evidence":`+updateEvidence+`}`, http.StatusCreated, &workspace)
