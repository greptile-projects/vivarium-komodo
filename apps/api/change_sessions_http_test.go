...
func TestCollaboratorDelegatesBoundedRunAndRevokesCredential(t *testing.T) {
	...
	body := `{"instructions":"Add retry coverage and keep the public API stable.","revision_id":"captured-source","context_paths":["apps/api","docs/README.md"],"working_branch":"candidate"}`
	...
	if res.Code != 201 || delegated.Run.InitiatorID != "collaborator" || delegated.Run.RevisionID != "captured-source" || delegated.Run.WorkingBranch != "candidate" || delegated.Credential.RepositoryID != string(repository.ID) || delegated.Credential.Branch != "refs/heads/candidate" {
	...
	if err != nil || grant.RepositoryID != string(repository.ID) || grant.Branch != "refs/heads/candidate" {
...
func TestWorkerPublishesPushedChangeIntoPullRequestReview(t *testing.T) {
	gitStorage, _ := storage.New(t.TempDir())
	catalog, _ := repositories.New(t.TempDir(), gitStorage)
	pulls, _ := pullrequests.New(t.TempDir())
	sessions, _ := changesessions.New(t.TempDir())
	...
	baseBlob, _ := opened.WriteObject(storage.BlobObject, []byte("before\n"))
	baseTree, _ := opened.WriteObject(storage.TreeObject, testTree(t, map[string]storage.ObjectID{"README.md": baseBlob}))
	base := writeCommit(t, opened, baseTree, nil, "Base")
	changedBlob, _ := opened.WriteObject(storage.BlobObject, []byte("after\n"))
	changedTree, _ := opened.WriteObject(storage.TreeObject, testTree(t, map[string]storage.ObjectID{"README.md": changedBlob}))
	changed := writeCommit(t, opened, changedTree, []storage.ObjectID{base}, "Agent update")
	_ = opened.CreateReference(storage.Reference{Name: "refs/heads/main", ObjectID: base})
	_ = opened.CreateReference(storage.Reference{Name: "refs/heads/candidate", ObjectID: base})
	pull, _ := pulls.Create(pullrequests.CreateParams{RepositoryID: string(repository.ID), AuthorID: "owner", Title: "Change", SourceBranch: "candidate", TargetBranch: "main", SourceCommitID: string(base), TargetCommitID: string(base)})
	_, _ = pulls.PutReview(string(repository.ID), pull.ID, "owner", pullrequests.Approve, string(base))
	session, _ := sessions.Create(string(repository.ID), pull.ID, "owner", string(base))
	issued, _ := credentials.IssueRepositoryGit("owner", "worker", string(repository.ID), "refs/heads/candidate", 24*time.Hour)
	run, _ := sessions.Delegate(string(repository.ID), pull.ID, session.ID, changesessions.DelegateParams{InitiatorID: "owner", Agent: "codex", Instructions: "Update docs", RevisionID: string(base), WorkingBranch: "candidate", CredentialGrantID: issued.ID, CredentialExpiresAt: issued.ExpiresAt})
	_, _ = sessions.AppendRunEvent(string(repository.ID), pull.ID, session.ID, run.ID, "run.started", map[string]string{"status": "working"})
	_ = opened.UpdateReference(storage.Reference{Name: "refs/heads/candidate", ObjectID: changed})

	mux := http.NewServeMux()
	registerChangeSessionsHTTP(mux, sessions, pulls, catalog, credentials, nil)
	path := "/repositories/" + string(repository.ID) + "/pull-requests/" + pull.ID + "/change-sessions/" + session.ID + "/runs/" + run.ID + "/publication"
	req := httptest.NewRequest(http.MethodPost, path, strings.NewReader(`{"summary":"Updated the guide.","checks":["go test ./..."],"concerns":["Follow-up wording review"]}`))
	req.Header.Set("Authorization", "Bearer "+issued.Token)
	res := httptest.NewRecorder()
	mux.ServeHTTP(res, req)
	if res.Code != http.StatusCreated {
		t.Fatalf("publication: %d %s", res.Code, res.Body.String())
	}
	updated, _ := pulls.Get(string(repository.ID), pull.ID)
	stored, _ := sessions.Get(string(repository.ID), pull.ID, session.ID)
	publication := stored.Runs[0].Publication
	if updated.SourceCommitID != string(changed) || stored.Runs[0].State != changesessions.Succeeded || publication == nil || len(publication.CommitIDs) != 1 || publication.CommitIDs[0] != string(changed) || len(publication.ChangedFiles) != 1 || publication.ChangedFiles[0] != "README.md" || publication.Checks[0] != "go test ./..." || publication.Concerns[0] != "Follow-up wording review" {
		t.Fatalf("published pull %#v session %#v", updated, stored)
	}
	reviews, _ := pulls.ListReviews(string(repository.ID), pull.ID)
	if len(reviews) != 1 || reviews[0].CommitID != string(base) {
		t.Fatalf("review attribution changed: %#v", reviews)
	}
}
