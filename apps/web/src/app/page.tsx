...
type SecurityReport = { id:string; title:string; summary:string; reporter_id:string; contact:{channel:string;value:string}; affected_repositories:{repository_id:string;versions?:string[]}[]; evidence:{title:string;kind:string;description:string}[]; severity:string; embargo_state:string; response_team:{user_id:string;invited_by_id:string;invited_at:string}[]; messages:{id:string;author_id:string;body:string;created_at:string}[]; audit_log:{sequence:number;type:string;actor_id:string;subject_id?:string;detail?:string;created_at:string}[]; created_at:string; updated_at:string };
...
export default function Home() {
  ...
  const [view, setView] = useState<"workspace" | "repositories" | "inbox" | "security" | "access">("workspace");

  const loadWorkspace = useCallback(async () => {
    try {
      const current = await api<Session>("/session");
      setSession(current);
      const [repoData, publicData, grantData, inboxData, securityData] = await Promise.all([api<Envelope<Repository>>("/repositories?affiliation=all&per_page=100"), api<Envelope<Repository>>("/repositories/public?per_page=100"), api<Envelope<Grant>>("/access-grants?per_page=100"), api<Envelope<InboxItem>>("/inbox?per_page=100"), api<Envelope<SecurityReport>>("/security-reports?per_page=100")]);
      setRepositories(repoData.items); setPublicRepositories(publicData.items); setGrants(grantData.items); setInbox(inboxData.items); setSecurityReports(securityData.items);
    } catch { setSession(null); setRepositories([]); setPublicRepositories([]); setGrants([]); setInbox([]); setSecurityReports([]); }
  }, []);
  ...
  return <Dashboard session={session} repositories={repositories} publicRepositories={publicRepositories} grants={grants} inbox={inbox} securityReports={securityReports} view={view} setView={setView}
...
function Dashboard({ session, repositories, publicRepositories, grants, inbox, securityReports, view, setView, refresh, onSignedOut }: { session: Session; repositories: Repository[]; publicRepositories: Repository[]; grants: Grant[]; inbox: InboxItem[]; securityReports: SecurityReport[]; view: "workspace"|"repositories"|"inbox"|"security"|"access"; setView: (view: "workspace"|"repositories"|"inbox"|"security"|"access") => void; refresh: () => Promise<void>; onSignedOut: () => void }) {
  ...
  return <WorkspaceShell displayName={session.user.display_name} handle={session.user.handle} initials={initials} repositoryCount={repositories.length} inboxCount={inbox.length} view={view} query={query} onQuery={setQuery} onView={setView} onCreate={() => setShowCreate(true)} onSignOut={signOut}>
    {view === "access" ? <Access grants={grants} refresh={refresh}/> : view === "inbox" ? <Inbox items={inbox} refresh={refresh}/> : view === "security" ? <SecurityReports actor={session.user.id} repositories={[...repositories,...publicRepositories.filter(item=>!repositories.some(repository=>repository.id===item.id))]} items={securityReports} refresh={refresh}/> : <Repositories user={session.user} repositories={filtered} discoverable={discoverable} total={repositories.length} searching={Boolean(query)} showCreate={showCreate} setShowCreate={setShowCreate} refresh={refresh} full={view === "repositories"}/>}
  </WorkspaceShell>;
}

function SecurityReports({actor,repositories,items,refresh}:{actor:string;repositories:Repository[];items:SecurityReport[];refresh:()=>Promise<void>}) {
  const [selected,setSelected]=useState(""); const [report,setReport]=useState<SecurityReport|null>(null); const [creating,setCreating]=useState(false); const [error,setError]=useState(""); const [busy,setBusy]=useState(false);
  const open=useCallback(async(id:string)=>{setSelected(id);setError("");try{setReport(await api<SecurityReport>(`/security-reports/${id}`));}catch(cause){setReport(null);setError(cause instanceof Error?cause.message:"Report unavailable.");}},[]);
  const maintainer=Boolean(report?.affected_repositories.some(affected=>repositories.some(repository=>repository.id===affected.repository_id&&repository.owner_id===actor)));
  async function create(event:FormEvent<HTMLFormElement>){event.preventDefault();setBusy(true);setError("");const data=new FormData(event.currentTarget);const repositoryIDs=data.getAll("repository").map(String);const versions=String(data.get("versions")).split("\n").map(value=>value.trim()).filter(Boolean);try{const item=await api<SecurityReport>("/security-reports",{method:"POST",body:JSON.stringify({title:data.get("title"),summary:data.get("summary"),contact:{channel:data.get("channel"),value:data.get("contact")},affected_repositories:repositoryIDs.map(repository_id=>({repository_id,versions})),evidence:[{title:data.get("evidence_title"),kind:data.get("evidence_kind"),description:data.get("evidence")}]})});setCreating(false);await refresh();await open(item.id);}catch(cause){setError(cause instanceof Error?cause.message:"Report could not be created.");}finally{setBusy(false)}}
  async function mutate(path:string,method:string,body?:unknown){if(!report)return;setBusy(true);setError("");try{const updated=await api<SecurityReport>(`/security-reports/${report.id}${path}`,{method,body:body===undefined?undefined:JSON.stringify(body)});setReport(updated);await refresh();}catch(cause){setError(cause instanceof Error?cause.message:"Change was not accepted.");}finally{setBusy(false)}}
  return <div className="security-workspace"><div className="eyebrow"><Shield size={14}/> Confidential coordination</div><div className="page-heading"><div><h1>Security reports</h1><p>A private, audited channel for suspected vulnerabilities. Reports never enter repository activity, search, or notifications.</p></div><Button onClick={()=>setCreating(value=>!value)}><Plus size={16}/>Report vulnerability</Button></div>
    {creating&&<form className="panel security-form" onSubmit={create}><label>Report title<input name="title" required maxLength={200}/></label><label>Safe contact channel<select name="channel"><option value="email">Email</option><option value="signal">Signal</option><option value="matrix">Matrix</option><option value="other">Other</option></select></label><label>Contact address<input name="contact" required maxLength={500}/></label><fieldset><legend>Affected repositories</legend>{repositories.map(repository=><label key={repository.id}><input type="checkbox" name="repository" value={repository.id}/>{repository.name}</label>)}</fieldset><label className="wide">Affected versions <small>One version or range per line; applied to the selected repositories.</small><textarea name="versions" required rows={3}/></label><label className="wide">What you observed<textarea name="summary" required rows={5}/></label><label>Evidence type<select name="evidence_kind"><option value="reproduction">Reproduction</option><option value="description">Description</option><option value="log">Redacted log</option><option value="artifact">Artifact reference</option><option value="reference">External reference</option></select></label><label>Evidence title<input name="evidence_title" required maxLength={300}/></label><label className="wide">Evidence details<textarea name="evidence" required rows={5}/></label>{error&&<p className="form-error wide">{error}</p>}<div className="form-actions wide"><Button variant="secondary" type="button" onClick={()=>setCreating(false)}>Cancel</Button><Button type="submit" disabled={busy}>Submit privately</Button></div></form>}
    <div className="security-layout"><aside className="panel security-list">{items.map(item=><button className={selected===item.id?"active":""} onClick={()=>void open(item.id)} key={item.id}><span><strong>{item.title}</strong><small>{new Date(item.updated_at).toLocaleString()}</small></span><Badge tone={item.embargo_state==="active"?"accent":"neutral"}>{item.severity} · {item.embargo_state}</Badge></button>)}{!items.length&&<p>No private reports are available to you.</p>}</aside>
      {report&&<article className="panel security-detail"><header><div><p className="eyebrow">Private report · {report.id.slice(0,8)}</p><h2>{report.title}</h2></div><span><Badge tone="accent">{report.severity}</Badge> <Badge>{report.embargo_state}</Badge></span></header><section><p className="security-summary">{report.summary}</p><dl><div><dt>Reporter contact</dt><dd>{report.contact.channel}: {report.contact.value}</dd></div><div><dt>Affected scope</dt><dd>{report.affected_repositories.map(item=><span key={item.repository_id}>{repositories.find(repo=>repo.id===item.repository_id)?.name??item.repository_id}: {item.versions?.join(", ")}</span>)}</dd></div></dl></section>
      {maintainer&&<section className="security-controls"><label>Severity<select value={report.severity} onChange={event=>void mutate("/triage","PATCH",{severity:event.target.value})}>{["unknown","low","medium","high","critical"].map(value=><option key={value}>{value}</option>)}</select></label><label>Embargo<select value={report.embargo_state} onChange={event=>void mutate("/triage","PATCH",{embargo_state:event.target.value})}><option value="requested">Requested</option><option value="active">Active</option><option value="lifted">Lifted</option></select></label><form onSubmit={event=>{event.preventDefault();const form=event.currentTarget;void mutate("/team","POST",{user_id:new FormData(form).get("user_id")}).then(()=>form.reset())}}><label>Invite responder by user ID<input name="user_id" required/></label><Button type="submit" disabled={busy}>Invite</Button></form></section>}
      <section className="security-section"><h3>Submitted evidence</h3>{report.evidence.map((item,index)=><article key={`${item.title}-${index}`}><Badge>{item.kind}</Badge><strong>{item.title}</strong><p>{item.description}</p></article>)}</section><section className="security-section"><h3>Bounded response team</h3>{report.response_team.length?<ul>{report.response_team.map(member=><li key={member.user_id}><code>{member.user_id}</code>{maintainer&&<button onClick={()=>void mutate(`/team/${member.user_id}`,"DELETE")}>Revoke access</button>}</li>)}</ul>:<p>No additional responders have access.</p>}</section>
      <section className="security-section"><h3>Private conversation</h3><ol>{report.messages.map(message=><li key={message.id}><p>{message.body}</p><small>{message.author_id===actor?"You":message.author_id} · {new Date(message.created_at).toLocaleString()}</small></li>)}</ol><form onSubmit={event=>{event.preventDefault();const form=event.currentTarget;void mutate("/messages","POST",{body:new FormData(form).get("body")}).then(()=>form.reset())}}><textarea name="body" required rows={3} placeholder="Share a private update with the reporter and response team"/><Button type="submit" disabled={busy}>Send privately</Button></form></section><details className="security-audit"><summary>Access audit · {report.audit_log.length} events</summary><ol>{[...report.audit_log].reverse().map(event=><li key={event.sequence}><strong>{event.type.replaceAll("."," ")}</strong><span>{event.actor_id}{event.subject_id&&` → ${event.subject_id}`}</span><time>{new Date(event.created_at).toLocaleString()}</time></li>)}</ol></details></article>}
    </div>{error&&!creating&&<p className="form-error">{error}</p>}</div>;
}
