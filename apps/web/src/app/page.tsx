...
type Repository = { id: string; name: string; description: string; visibility: "private" | "public"; empty: boolean; git_url: string; updated_at: string };
...
export default function Home() {
  ...
  const loadWorkspace = useCallback(async () => {
    try {
      ...
      setSession(current);
      const [repoData, grantData] = await Promise.all([api<Envelope<Repository>>("/repositories?per_page=100"), api<Envelope<Grant>>("/access-grants?per_page=100")]);
...
function Repositories({ user, repositories, total, searching, showCreate, setShowCreate, refresh, full }: { user: UserRecord; repositories: Repository[]; total: number; searching: boolean; showCreate: boolean; setShowCreate: (show: boolean) => void; refresh: () => Promise<void>; full: boolean }) {
  return <><div className="eyebrow"><Sparkles size={14}/>{total ? "Your workspace" : "One step from collaboration"}</div><div className="page-heading"><div><h1>{total ? `${full ? "Repositories" : "Welcome back"}, ${user.display_name.split(" ")[0]}.` : `Welcome, ${user.display_name.split(" ")[0]}.`}</h1><p>{total ? "Find a project or make room for the next idea." : "Create your first repository to give your work a home."}</p></div><Button onClick={() => setShowCreate(true)}><Plus size={16}/>New repository</Button></div>
    ...
    {repositories.length ? <section aria-labelledby="repository-heading"><div className="section-heading"><div><h2 id="repository-heading">{searching ? "Matching repositories" : "Your repositories"}</h2><p>{repositories.length} {repositories.length === 1 ? "project" : "projects"} ready for collaboration</p></div></div><div className="repo-grid">{repositories.map(repo => <RepositoryTile key={repo.id} repository={repo}/>)}</div></section> : total ? <EmptySearch/> : <FirstRepository onCreate={() => setShowCreate(true)}/>}</>;
}
...
function CreateRepository({ close, refresh }: { close: () => void; refresh: () => Promise<void> }) {
  ...
  return <section className="create-panel panel" aria-labelledby="create-title"><div className="create-copy"><span className="repo-icon"><Book/></span><div><h2 id="create-title">Create a repository</h2><p>Give the project a clear name. You can add collaborators after the first push.</p></div></div><form className="repository-form" onSubmit={submit}><label>Repository name<input name="name" required pattern="[a-z0-9._-]+" maxLength={100} placeholder="project-name"/></label><label>Description <span>Optional</span><textarea name="description" maxLength={280} placeholder="What are you building together?"/></label><fieldset><legend>Visibility</legend><label className="radio-card"><input type="radio" name="visibility" value="private" defaultChecked/><Shield size={17}/><span><strong>Private</strong><small>Only you and invited contributors</small></span></label><label className="radio-card"><input type="radio" name="visibility" value="public"/><Book size={17}/><span><strong>Public</strong><small>Anyone can view and clone</small></span></label></fieldset>{error && <p className="form-error" role="alert">{error}</p>}<div className="form-actions"><Button type="button" variant="secondary" onClick={close}>Cancel</Button><Button type="submit" disabled={pending}>{pending ? "Creating…" : "Create repository"}</Button></div></form></section>;
}

function RepositoryTile({ repository }: { repository: Repository }) {
