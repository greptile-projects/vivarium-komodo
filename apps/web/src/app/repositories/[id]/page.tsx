...
type Deployment = { id: string; environment_id: string; release_id: string; build_run_id: string; artifact_id: string; artifact_path: string; artifact_sha256: string; source_commit_id: string; state: "pending"|"queued"|"running"|"succeeded"|"failed"; initiated_by_id: string; approvals: Array<{actor_id:string;created_at:string}>; events: Array<{sequence:number;type:string;state?:string;actor_id?:string;stream?:string;message?:string;created_at:string}>; created_at:string };
...
function ReleaseWorkspace({ repository, branches, actor, selected }: { repository: Repository; branches: BranchRecord[]; actor: string; selected?: string }) {
  ...
  return <div className="release-workspace">
    ...
    {items.length === 0 && !creating ? <section className="panel release-empty"><Sparkles/><h3>No release candidates yet</h3><p>Select an exact branch tip and capture the work that led to it.</p></section> : <div className="release-layout">
      ...
      {deployments.filter((deployment)=>deployment.release_id===current.id).map((deployment)=>{const environment=environments.find((item)=>item.id===deployment.environment_id);const logs=deployment.events.filter((event)=>event.type==="log");return <details className="deployment-record" open={deployment.state==="running"||deployment.state==="pending"} key={deployment.id}><summary><span><strong>{environment?.name??"Environment"}</strong><small>{deployment.artifact_path} · initiated by <Actor id={deployment.initiated_by_id}/></small></span><Badge tone={deployment.state==="succeeded"?"accent":"neutral"}>{deployment.state}</Badge></summary><div><p>Artifact <code>{deployment.artifact_sha256}</code></p><p>Approvals: {deployment.approvals.length?deployment.approvals.map((approval)=><Actor id={approval.actor_id} key={approval.actor_id}/>):"None yet"}</p>{deployment.state==="pending"&&deployment.initiated_by_id!==actor&&authorized&&<Button variant="secondary" disabled={deliveryBusy===deployment.id} onClick={()=>void approve(deployment)}>Approve exact artifact</Button>}{logs.length>0&&<pre aria-label="Deployment logs">{logs.map((event)=>event.message).join("")}</pre>}<ol className="deployment-timeline">{deployment.events.filter((event)=>event.type!=="log").map((event)=><li key={event.sequence}><strong>{event.type}{event.state?` · ${event.state}`:""}</strong>{event.actor_id&&<> by <Actor id={event.actor_id}/></>}<small>{new Date(event.created_at).toLocaleString()}</small></li>)}</ol></div></details>})}</article>
