...
function ReleaseWorkspace({ repository, branches, actor, selected }: { repository: Repository; branches: BranchRecord[]; actor: string; selected?: string }) {
  ...
  return <div className="release-workspace">
    ...
    {items.length === 0 && !creating ? <section className="panel release-empty"><Sparkles/><h3>No release candidates yet</h3><p>Select an exact branch tip and capture the work that led to it.</p></section> : <div className="release-layout">
      ...
      {deployments.filter((deployment)=>deployment.release_id===current.id).map((deployment)=>{const environment=environments.find((item)=>item.id===deployment.environment_id);const logs=deployment.events.filter((event)=>event.type==="log");const active=["pending","queued","running","paused"].includes(deployment.state);return <details id={`deployment-${deployment.id}`} className="deployment-record" open={active} key={deployment.id}><summary><span><strong>{environment?.name??"Environment"}{deployment.current_stage?` · ${deployment.current_stage}`:""}</strong><small>{deployment.artifact_path} · initiated by <Actor id={deployment.initiated_by_id}/></small></span><Badge tone={deployment.state==="succeeded"?"accent":"neutral"}>{deployment.state}</Badge></summary><div><p>Affected revision <code>{deployment.source_commit_id}</code></p><p>Artifact <code>{deployment.artifact_sha256}</code></p><p>Approvals: {deployment.approvals.length?deployment.approvals.map((approval)=><Actor id={approval.actor_id} key={approval.actor_id}/>):"None yet"}</p>{deployment.state==="pending"&&deployment.initiated_by_id!==actor&&authorized&&<Button variant="secondary" disabled={deliveryBusy===deployment.id} onClick={()=>void approve(deployment)}>Approve exact artifact</Button>}{authorized&&active&&<div className="release-actions">{deployment.state==="running"&&<Button variant="secondary" disabled={deliveryBusy===deployment.id} onClick={()=>void control(deployment,"pause")}>Pause rollout</Button>}{deployment.state==="paused"&&<Button variant="secondary" disabled={deliveryBusy===deployment.id} onClick={()=>void control(deployment,"resume")}>Resume rollout</Button>}<Button variant="secondary" disabled={deliveryBusy===deployment.id} onClick={()=>void control(deployment,"cancel")}>Cancel</Button>{(deployment.state==="running"||deployment.state==="paused")&&<Button variant="danger" disabled={deliveryBusy===deployment.id} onClick={()=>void control(deployment,"fail")}>Mark unsuccessful</Button>}</div>}{deployment.decision_by_id&&<p>Latest decision by <Actor id={deployment.decision_by_id}/>{deployment.decision_reason?`: ${deployment.decision_reason}`:""}</p>}{logs.length>0&&<pre aria-label="Deployment logs">{logs.map((event)=>event.message).join("")}</pre>}<ol className="deployment-timeline">{deployment.events.filter((event)=>event.type!=="log").map((event)=><li key={event.sequence}><strong>{event.type}{event.stage?` · ${event.stage}`:""}{event.signal?` · ${event.signal}`:""}{event.outcome?` · ${event.outcome}`:""}{event.state?` · ${event.state}`:""}</strong>{event.actor_id&&<> by <Actor id={event.actor_id}/></>}{event.message&&<span>{event.message}</span>}<small>{new Date(event.created_at).toLocaleString()}</small></li>)}</ol></div></details>})}</article>
