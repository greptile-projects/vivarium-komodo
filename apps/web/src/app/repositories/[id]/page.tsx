...
import { RunbookRehearsals } from "@/components/runbook-rehearsals";
import { RunbookExecutions } from "@/components/runbook-executions";
...
import { WorkflowComponents } from "@/components/workflow-components";
  ...
  return (
    <RepositoryFrame>
      ...
      {view === "assurance" ? (
        ...
        <><DesignProposals repository={repository.id} actor={actor} /><DesignSystems repository={repository.id} actor={actor} /></>
      ) : view === "documentation" ? (
        ...
        <><RunbookExecutions repository={repository.id} actor={actor} /><Runbooks repository={repository.id} /><RunbookRehearsals repository={repository.id} /><ResponsePolicies repository={repository.id} actor={actor} /><ResponseRotations repository={repository.id} actor={actor} /><ResponseAlerts repository={repository.id} actor={actor} /><ResponseOutcomes repository={repository.id} actor={actor} /></>
      ) : view === "accessibility" ? (
...
function ConflictEvidence({ repository, pull, actor, analysis }: { repository:string; pull:PullRequest; actor:string; analysis: ConflictAnalysis }) {
  const [workspace,setWorkspace]=useState<string>();
  const [launching,setLaunching]=useState(false);
  const [launchError,setLaunchError]=useState("");
  const launch=async()=>{setLaunching(true);setLaunchError("");try{const item=await send<{id:string}>(`/repositories/${repository}/pull-requests/${pull.id}/conflicts/workspace`,"POST",{});setWorkspace(item.id)}catch(cause){setLaunchError(cause instanceof Error?cause.message:"Workspace could not be launched.")}finally{setLaunching(false)}};
