...
type Question = {
  ...
  improvements: Array<{id:string;classification:string;acceptance_criteria:string[];target_kind:string;target_id:string;links:Array<{kind:string;resource_id:string;state:string;revision?:string;summary?:string}>;created_by_id:string}>;
};
...
export function SupportQuestions({
  ...
  async function improve(e:FormEvent<HTMLFormElement>){e.preventDefault();if(!current)return;const f=new FormData(e.currentTarget),classification=String(f.get("classification")),target_kind=classification==="defect"||classification==="compatibility_problem"?"issue":"proposal";try{const out=await api<{question:Question}>(`/repositories/${repository}/support-questions/${current.id}/improvements`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({classification,target_kind,title:f.get("title"),visibility:current.audience==="public"?"public":"repository",acceptance_criteria:String(f.get("criteria")).split("\n").filter(Boolean),discussion_ids:f.getAll("discussion_ids"),work:target_kind==="proposal"?[{title:f.get("work_title"),owner_kind:f.get("owner_kind"),owner_id:f.get("owner_id"),acceptance_criteria:String(f.get("criteria")).split("\n").filter(Boolean)}]:[]})});setCurrent(out.question);e.currentTarget.reset()}catch(c){setError(c instanceof Error?c.message:"Could not create accountable improvement")}}
  if (current)
    return (
      <section className="workspace">
        ...
        <div className="card">
          <h3>Accountable improvement</h3>
          <p className="muted">A collaborator can carry only selected thread context into ordinary governed work. This does not grant assignment, agent, repository, review, or merge authority.</p>
          {current.improvements?.map(x=><article className="panel" key={x.id}><p><Badge>{x.classification.replaceAll("_"," ")}</Badge> {x.target_kind.replaceAll("_"," ")} <code>{x.target_id}</code> · opened by {x.created_by_id}</p><p><strong>Acceptance:</strong> {x.acceptance_criteria.join(" · ")}</p>{x.links.map((link,i)=><p key={i}><Badge>{link.state.replaceAll("_"," ")}</Badge> {link.kind.replaceAll("_"," ")} <code>{link.resource_id}</code> {link.summary}</p>)}</article>)}
          {actor&&<details><summary>The current answer is not good enough</summary><form className="form-stack" onSubmit={improve}><label>Classify the gap<select name="classification"><option value="defect">Defect</option><option value="documentation_gap">Documentation gap</option><option value="missing_example">Missing example</option><option value="compatibility_problem">Compatibility problem</option><option value="product_opportunity">Product opportunity</option></select></label><label>Improvement title<input name="title" required/></label><label>Acceptance criteria, one per line<textarea name="criteria" required/></label><label>First work item (for guidance/product work)<input name="work_title" defaultValue="Deliver and verify the requested improvement"/></label><label>Owner kind<select name="owner_kind"><option value="human">Human</option><option value="agent">Agent</option></select></label><label>Proposed owner ID<input name="owner_id"/></label><fieldset><legend>Permitted discussion to preload</legend>{current.discussion.map(x=><label key={x.id}><input type="checkbox" name="discussion_ids" value={x.id}/> {x.author_id}: {x.body}</label>)}</fieldset><Button type="submit">Create governed work</Button></form></details>}
        </div>
        <div className="card">
          <h3>Discussion</h3>
